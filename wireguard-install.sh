#!/bin/bash
#
# https://github.com/Namiz4n/vpn-installation
#
...
check_nftables() {
	if [ "$os" = "centos" ]; then
		if grep -qs "Namizan VPN script" /etc/sysconfig/nftables.conf \
...
GitHub: https://github.com/Namiz4n/vpn-installation
EOF
}
...
select_dns() {
    ...
    if [ "$auto" = 0 ]; then
        echo
        echo -e "${YELLOW}+---------------------------------------------+${NC}"
        echo -e "${GREEN}|         Select a DNS server for client       |${NC}"
        echo -e "${YELLOW}+---------------------------------------------+${NC}"
        echo -e "${BLUE}|${YELLOW} 1.${NC} ${CYAN}Current system resolvers${NC}         ${BLUE}|${NC}"
        echo -e "${BLUE}|${YELLOW} 2.${NC} ${CYAN}Google Public DNS${NC}               ${BLUE}|${NC}"
        echo -e "${BLUE}|${YELLOW} 3.${NC} ${CYAN}Cloudflare DNS${NC}                  ${BLUE}|${NC}"
        echo -e "${BLUE}|${YELLOW} 4.${NC} ${CYAN}OpenDNS${NC}                         ${BLUE}|${NC}"
        echo -e "${BLUE}|${YELLOW} 5.${NC} ${CYAN}Quad9${NC}                           ${BLUE}|${NC}"
        echo -e "${BLUE}|${YELLOW} 6.${NC} ${CYAN}AdGuard DNS${NC}                     ${BLUE}|${NC}"
        echo -e "${BLUE}|${YELLOW} 7.${NC} ${CYAN}Custom${NC}                          ${BLUE}|${NC}"
        echo -e "${YELLOW}+---------------------------------------------+${NC}"
        echo

        read -rp "$(echo -e ${GREEN}DNS server [2]:${NC} ) " dns
        until [[ -z "$dns" || "$dns" =~ ^[1-7]$ ]]; do
            echo -e "${RED}$dns: invalid selection.${NC}"
            read -rp "$(echo -e ${GREEN}DNS server [2]:${NC} ) " dns
        done
    else
        dns=2
...
select_menu_option() {
    ...
    echo -e "${YELLOW}+------------------------------------------+${NC}"
    echo -e "${BLUE}|${YELLOW} 1.${NC} ${CYAN}Add a new client${NC}                  ${BLUE}|${NC}"
    echo -e "${BLUE}|${YELLOW} 2.${NC} ${CYAN}List existing clients${NC}            ${BLUE}|${NC}"
    echo -e "${BLUE}|${YELLOW} 3.${NC} ${CYAN}Remove an existing client${NC}         ${BLUE}|${NC}"
    echo -e "${BLUE}|${YELLOW} 4.${NC} ${CYAN}Show QR code for a client${NC}         ${BLUE}|${NC}"
    echo -e "${BLUE}|${YELLOW} 5.${NC} ${RED}Remove WireGuard${NC}                  ${BLUE}|${NC}"
    echo -e "${BLUE}|${YELLOW} 6.${NC} ${CYAN}Exit${NC}                              ${BLUE}|${NC}"
    echo -e "${YELLOW}+------------------------------------------+${NC}"
    echo

    read -rp "$(echo -e ${GREEN}Option:${NC} ) " option
    until [[ "$option" =~ ^[1-6]$ ]]; do
        echo -e "${RED}$option: invalid selection.${NC}"
        read -rp "$(echo -e ${GREEN}Option:${NC} ) " option
    done
}
